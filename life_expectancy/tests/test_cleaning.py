...
def test_clean_data_TSV(eu_life_expectancy_raw, pt_life_expectancy_expected, region: str = Country.PT, type_strategy: TypeStrategy = TSVStrategy()) -> None:
...
def test_clean_data_JSON(eurostat_life_expect, pt_life_expectancy_expected, region: str = Country.PT, type_strategy: TypeStrategy = JSONStrategy()) -> None:
    """Run the `clean_data` function and compare the output to the expected output"""

    actual = clean_data(eurostat_life_expect, region = region, type_strategy=type_strategy).reset_index(drop=True)
